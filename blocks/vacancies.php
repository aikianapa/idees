<view><section class="vacancies container">
  <div class="vacancies__title-container">
    <h1 class="page__title">Станьте частью нашей команды и создавайте с нами уникальные бренды и лучшие сервисы</h1>
    <wb-var tags=""></wb-var>
    ...
    <ul class="vacancies__tags tags">
      <li class="tags__item">
        <button class="tag tag--active" type="button" data-ajax="{'target':'#vacanciesListing','filter_remove': 'tags' }">Все вакансии</button>
      </li>
      <wb-foreach wb-from="_var.tags">
      <li class="tags__item">
        <button class="tag" type="button" data-ajax="{'target':'#vacanciesListing','filter_add': { 'tags': {'$like' : '{{_val}}'} } }">{{_val}}</button>
      </li>
      </wb-foreach>
    </ul>
  </div>
  ...
  <ul class="vacancies__list" id="vacanciesListing">
    ...
    <li class="vacancies__item" data-tags="{{tags}}">
      <a class="vacancies__link" href="/vacancies/%7B%7BwbFurlGenerate(%7B%7Bname%7D%7D)%7D%7D">
...
</section><wb-snippet wb="name=wbapp"></wb-snippet><script wb-app>
    $('.vacancies__tags button').on('click',function(){
        $('.vacancies__tags button').removeClass('tag--active');
        $(this).addClass('tag--active');
    })
</script></view><edit header="Список вакансий"><div>
    <wb-module wb="module=yonger&amp;mode=edit&amp;block=common.inc"></wb-module>
  </div>
<wb-include wb="form=vacancies&amp;mode=list"></wb-include></edit>